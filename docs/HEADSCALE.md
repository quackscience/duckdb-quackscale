...
Uses the [official Headscale container](https://headscale.net/stable/setup/install/container/) (`docker.io/headscale/headscale:0.28.0`) — no custom images. Started after checkout on Docker network `quacktail-ci` with hostname alias **`headscale`**. Control URL: **`http://headscale:8080`**.
